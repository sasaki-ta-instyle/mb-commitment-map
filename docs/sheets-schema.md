...
| H | `url` / `リンク` | text (任意) | 詳細ボックスをクリックすると別窓で開くリンク。Notion / Google Drive / 外部URL 何でも。空なら箱はクリック不可 |
| I | `連携` | text | この行から部署をまたいで繋げたい先を列挙。書式は下記 |
| J | `owner` | text | 担当者名（`_members` タブから dropdown 参照） |
| K | `due_date` | date | 期日 (YYYY-MM-DD) |
| L | `progress` | 列挙 | `未着手` / `進行中` / `完了` / `停止` |
| M | `editor` / `最終編集者` | text | 最終編集者名（`_members` タブから dropdown、手で選択） |
| N | `updated_at` | date | 手入力（触ったときに更新） |
...
C-102|(未使用)                                       第2フィールド (label) は現状描画されない
本店/LP 転換率改善|(未使用)|dotted                     第3フィールドで style 指定
```
- ターゲットは `C-xxx`（他行の id）または `<部署>/<作戦|成果>`（別タブ `連携` と同じ書式）
- **ラベルは現状 hover 時に描画しない仕様** — 記入しても無視される。将来復活したら描画予定
- スタイル: `dashed`（既定）/ `dotted` / `solid`
- 別タブ `連携` と併用可（両方から集めて重複は自動除去）
- 現在の hover 挙動: 端点ボックスにマウスを当てるとその連携線だけがアクセントカラー (teal) の破線で表示される
...
| C | `ラベル` | text (任意) | 現状 hover 時に描画しない。将来復活時に使用予定 |
